...
# --- 0. GESTIONE UTENTI E AUTENTICAZIONE (SUPABASE TABELLA "utenti") ---
        ...
        if btn_login:
            ...
            if username_input == "Valerio" and (password_input == pwd_segreta or pwd_h == admin_pswd_stored or (admin_pswd_stored and password_input == admin_pswd_stored)):
                ...
                try:
                    ...
                    st.error(f"Errore di connessione alla tabella utenti: {e}")
    st.stop()
else:
...
DEFAULT_BANCA_DATI = [
    ...
    {"Alimento": "biscotti gocciole", "gr/n": 1, "carbo": 7.7, "proteine": 0.8, "grassi": 2.6, "kcal": 59},
    ...
    {"Alimento": "semi di zucca", "gr/n": 13, "carbo": 1.43, "proteine": 3.9, "grassi": 6.37, "kcal": 72.67},
...
if "banca_dati_df" not in st.session_state:
    if dati_salvati and "banca_dati_df" in dati_salvati and dati_salvati["banca_dati_df"] is not None:
        if isinstance(dati_salvati["banca_dati_df"], list):
            st.session_state.banca_dati_df = pd.DataFrame(dati_salvati["banca_dati_df"])
        else:
...
if "atleta_corrente" not in st.session_state:
    if dati_salvati and "atleta_corrente" in dati_salvati and dati_salvati["atleta_corrente"] in st.session_state.atleti:
                        ...
                        if utente_da_eliminare is not None:
                            if st.button(f"Conferma ed Elimina '{utente_da_eliminare}'"):
                                try:
                                    ...
                                    salva_dati_disco()
                                    st.success(f"Operazione completata con successo: l'utente '{utente_da_eliminare}' è stato cancellato.")
                                    st.session_state.pop("select_utente_elimina", None)
                                    st.rerun()
                                except Exception as e:
...
if is_proprietario:
    atleta_selezionato = st.sidebar.selectbox(
        "Seleziona Atleta",
        lista_atleti,
        index=lista_atleti.index(st.session_state.atleta_corrente) if st.session_state.atleta_corrente in lista_atleti else 0,
...
genere_index = genere_opzioni.index(saved_genere) if saved_genere in genere_opzioni else 0
...
# --- LOGICA DI CALCOLO MACRONUTRIENTI (CONFERMA GIORNALIERA E PULSANTE DI CORREZIONE) ---
...
tot_grassi = sum([db_diario_atleta[data_str][p]["grassi"].sum() for p in PASTI if isinstance(db_diario_atleta.get(data_str, {}).get(p), pd.DataFrame) and not db_diario_atleta[data_str][p].empty])
...
with col_m4:
    ...
    progresso_grassi = float(min(tot_grassi / obj_grassi, 1.0)) if obj_grassi > 0 else 0.0
                ...
                try:
                    ...
                    if df_nuovo is not None and not df_nuovo.empty:
                        ...
                        if st.button("Conferma e Aggiungi alla Banca Dati"):
                            ...
                            st.session_state.banca_dati_df = (
                                pd.concat([st.session_state.banca_dati_df, df_finale], ignore_index=True)
...
st.subheader(f"Panoramica dei 6 Pasti Giornalieri - {st.session_state.atleta_corrente}")
...
for i, pasto in enumerate(PASTI):
    ...
    with col_target:
        with st.container(border=True):
            st.markdown(f"### {pasto}")
            df_p = pd.DataFrame(db_diario_atleta[data_str][pasto]) if not isinstance(db_diario_atleta[data_str][pasto], pd.DataFrame) else db_diario_atleta[data_str][pasto]
                ...
                if mostra_gestione_voci:
                    ...
                    with col_btn1:
                        if st.button("Elimina", key=f"btn_del_{pasto}"):
                            ...
                            db_diario_atleta[data_str][pasto] = df_p.drop(idx_to_drop).reset_index(drop=True)
                            salva_dati_disco()
                            st.rerun()
                    with col_btn2:
...
st.subheader(f"Esportazione Report in PDF - {st.session_state.atleta_corrente}")
    ...
    with col_pdf1:
        ...
        if st.button("Genera e Scarica PDF Giornaliero"):
            try:
                ...
                pdf_output.cell(0, 10, f"Report Nutrizionale - {st.session_state.atleta_corrente} ({data_str})", ln=True, align="C")
                pdf_output.ln(10)
        ...
        if st.button("Genera e Scarica PDF Intervallo (Traccia Totale)"):
            try:
                if data_inizio > data_fine:
                    ...
                    pdf_output.add_page()
                    pdf_output.set_font("Arial", "B", 16)
                    pdf_output.cell(0, 10, f"Report Nutrizionale - {st.session_state.atleta_corrente} ({data_inizio.strftime('%d/%m/%Y')} - {data_fine.strftime('%d/%m/%Y')})", ln=True, align="C")
                    pdf_output.ln(10)
                    ...
                    pdf_output.set_font("Arial", "B", 12)
                    pdf_output.set_text_color(0, 0, 0)
                    pdf_output.cell(0, 10, "Traccia Giornaliera dei Macronutrienti (Tutti i giorni):", ln=True)
                    pdf_output.set_font("Arial", "", 10)
