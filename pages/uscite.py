        ...
        if dati_sec4:
            ...
            if not df_s4.empty and 'start_date_local' in df_s4.columns:
                ...
                if not df_s4_filtrato.empty:
                    ...
                    # Estrazione diretta dai campi presenti nel tuo JSON
                    val_load = float(m.get('icu_training_load') or m.get('load') or 0.0)
                    val_eftp = float(m.get('icu_ftp') or m.get('eftp') or 279.0)
                    
                    # CTL e ATL presi preferibilmente dal JSON dell'attività se presenti, altrimenti dal wellness
                    ...
                    # W' Bal (gestione del campo ss_w_prime o simili presenti nel debug)
                    w_bal_raw = float(m.get('ss_w_prime') or m.get('min_w_prime_balance') or m.get('w_prime_balance') or m.get('wBal') or 0.0)
                    if abs(w_bal_raw) > 50:
