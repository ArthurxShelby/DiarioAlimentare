...
if not is_proprietario:
  st.error(
      "🚨 Accesso Negato: questa sezione è riservata esclusivamente al"
      " proprietario."
  )
  st.info(
      "Torna alla pagina principale del Diario Alimentare ed effettua il login"
      " con le credenziali da amministratore."
  )
  st.stop()
...
if resp_global.status_code == 200:
  ...
  if activities_net:
    ...
    tot_km = round(
        df_activities.get("distance", pd.Series([0])).fillna(0).sum() / 1000.0, 2
    )
    tot_dislivello = int(
        df_activities.get("total_elevation_gain", pd.Series([0]))
        .fillna(0)
        .sum()
    )
    ...
    st.markdown("---")
    st.subheader(
        "📊 Statistiche Dinamiche e Riepilogo (TCR - Dal 15/11/2025)"
    )
...
if "saved_start" not in st.session_state:
  st.session_state["saved_start"] = carica_data_salvata(
      FILE_DATA_INIZIO, date(2026, 1, 1)
  )

if "saved_end" not in st.session_state:
  st.session_state["saved_end"] = carica_data_salvata(
      FILE_DATA_FINE, date.today()
  )

with st.expander(
    "🔍 Esplora Archivio Storico da Intervals (Range Personalizzato e Filtri)",
    expanded=False,
):
  st.write(
      "Seleziona un periodo, filtra per data specifica o cerca per nome"
      " dell'uscita all'interno del flusso di Intervals."
  )
  ...
  with col_c1:
    data_inizio_custom = st.date_input(
        "Data Inizio Range",
        value=st.session_state["saved_start"],
        key="widget_start",
    )
  with col_c2:
    data_fine_custom = st.date_input(
        "Data Fine Range",
        value=st.session_state["saved_end"],
        key="widget_end",
    )
  ...
  with col_f1:
    filtro_nome = st.text_input(
        "Filtra per Nome Uscita (opzionale):",
        value="",
        placeholder="Es. Giro Samu, Salita...",
    )
  with col_f2:
  ...
  if attiva_data_singola:
    data_singola_specifica = st.date_input(
        "Seleziona Data Specifica", value=date.today(), key="widget_single_date"
    )
  ...
  if st.button("🚀 Estrai Dati dal Flusso", key="btn_calcola_custom"):
    ...
    with st.spinner("Interrogazione in corso..."):
      ...
      resp_custom = requests.get(
          url_custom, auth=auth_custom, params=params_custom
      )
      ...
      if resp_custom.status_code == 200:
        ...
        if attivita_ext_custom:
          ...
          st.info(
              "Nessuna attività trovata in questo intervallo nel flusso di"
              " Intervals."
          )
      else:
        st.error(
            "Errore di connessione a Intervals.icu:"
            f" {resp_custom.status_code}"
        )

  if (
      "custom_activities" in st.session_state
      and st.session_state["custom_activities"]
  ):
    ...
    if filtro_nome.strip():
      ...
      attivita_da_mostrare = [
          act
          for act in attivita_da_mostrare
    ...
    if not attivita_da_mostrare:
      ...
      distanza_tot_km = round(
          df_filtrato_vista.get("distance", pd.Series([0]))
          .fillna(0)
          .sum()
          / 1000.0,
          2,
      )
      dislivello_tot_m = int(
          df_filtrato_vista.get("total_elevation_gain", pd.Series([0]))
          .fillna(0)
          .sum()
      )
          ...
          with st.container(border=True):
            ...
            with col_info:
              st.markdown(
                  ...
                  f"{act_title} <span style='font-size: 1.1rem; color:"
                  f" #999;'>({act_date})</span></h3>",
              ...
              nuovo_nome_sec2 = st.text_input(
                  ...
                  value=act_title,
                  key=f"input_nome_sec2_{act_id}_{idx}",
              )
              if nuovo_nome_sec2 != act_title:
                if st.button(
                    "Salva Nome", key=f"btn_save_nome_sec2_{act_id}_{idx}"
                ):
              ...
              st.markdown(
                  ...
                  f"{act_dist} km</b> &nbsp;|&nbsp; D+: <b>{act_elev} m</b>"
                  f" &nbsp;|&nbsp; Tempo: <b>{act_time}</b></p>",
                  unsafe_allow_html=True,
              )
            with col_btn1:
              st.write("")
              btn_label = (
                  ...
                  else "🗺️ Anteprima Mappa"
              )
              if st.button(
                  btn_label,
                  key=f"btn_preview_{act_id}_{idx}",
            ...
            with col_btn2:
              ...
              if st.button(
                  "🔍 Pagina Dedicata",
                  key=f"btn_custom_{act_id}_{idx}",
                  use_container_width=True,
              ):
            ...
            if st.session_state[key_toggle]:
              ...
              target_url = (
                  f"https://intervals.icu/api/v1/activity/{clean_id}/streams"
              )
              ...
              with st.spinner("Caricamento tracciato in corso..."):
                try:
                  ...
                  if resp_streams.status_code == 404 and act_id != clean_id:
                    target_url = (
                        f"https://intervals.icu/api/v1/activity/{act_id}/streams"
                    )
                    resp_streams = requests.get(target_url, auth=auth_streams)
                  ...
                  if resp_streams.status_code == 200:
                      ...
                      for stream in data:
                        if isinstance(stream, dict):
                          ...
                          if stype in ["latlng", "lating"]:
                            ...
                            if (
                                isinstance(lat_data, list)
                                and isinstance(lon_data, list)
                                and len(lat_data) == len(lon_data)
                                and len(lat_data) > 0
                            ):
                            ...
                            elif isinstance(lat_data, list) and len(
                                lat_data
                            ) > 0:
                              for pt in lat_data:
                                if (
                    ...
                    if lats and lons:
                      ...
                      linee = [
                          ...
                          (
                              '<gpx version="1.1" creator="Streamlit App"'
                      ...
                      href = (
                          f'<a href="data:application/gpx+xml;base64,{b64}"'
                          f' download="{nome_file}.gpx" style="text-decoration:'
                          ' none;"><div style="background-color: #ff4b4b;'
                          " color: white; padding: 0.5rem 1rem; border-radius:"
                          " 0.5rem; text-align: center; font-weight: 600;"
                          " margin-top: 0.5rem; margin-bottom: 0.5rem;\">📥"
                          " Scarica Tracciato GPX</div></a>"
                      )
                      st.markdown(href, unsafe_allow_html=True)
                    else:
                      st.warning(
                          "Nessun punto di coordinate valido trovato in questa"
                          " attività."
                      )
                  else:
                    st.error(
                        "Errore nel recupero flussi da Intervals (Status:"
                        f" {resp_streams.status_code})"
                    )
                except Exception as e:
...
if "grafico_start_val" not in st.session_state:
  st.session_state["grafico_start_val"] = carica_data_salvata(
      FILE_GRAFICO_INIZIO, date(2026, 1, 1)
  )

if "grafico_end_val" not in st.session_state:
  st.session_state["grafico_end_val"] = carica_data_salvata(
      FILE_GRAFICO_FINE, date.today()
  )

with st.expander(
    "📈 Analisi Grafica e Dettaglio Uscite per Metrica", expanded=False
):
  st.write(
      "Fissa il range temporale di ricerca, il livello di aggregazione"
      " (Settimane/Mesi) e seleziona il parametro da analizzare."
  )

  col_r1, col_r2, col_r3, col_r4 = st.columns([2, 2, 2, 2])
  with col_r1:
    range_inizio = st.date_input(
        "Inizio Range Grafico",
        value=st.session_state["grafico_start_val"],
        key="grafico_start_input",
    )
  with col_r2:
    range_fine = st.date_input(
        "Fine Range Grafico",
        value=st.session_state["grafico_end_val"],
        key="grafico_end_input",
    )
  with col_r3:
  ...
  resp_grafico = requests.get(
      url_grafico, auth=("API_KEY", API_KEY.strip()), params=params_grafico
  )
  ...
  if resp_grafico.status_code == 200:
    ...
    if dati_raw_grafico:
      ...
      if tipo_aggregazione == "Settimanale":
        df_g["periodo_chiave"] = (
            df_g["data_fmt"].dt.to_period("W").dt.start_time.dt.date
        )
        df_aggregato = (
            df_g.groupby("periodo_chiave")
            .agg({
                "Km": "sum",
                "D+": "sum",
                "Ore in sella": "sum",
                "titolo_uscita": lambda x: (
                    f"Totale Settimanale ({len(x)} uscite)"
                ),
            })
            .reset_index()
            .rename(columns={"periodo_chiave": "asse_x"})
        )
      elif tipo_aggregazione == "Mensile":
        df_g["periodo_chiave"] = (
            df_g["data_fmt"].dt.to_period("M").dt.start_time.dt.date
        )
        df_aggregato = (
            df_g.groupby("periodo_chiave")
            .agg({
      ...
      event_selezionato = st.plotly_chart(
          fig_stat,
          use_container_width=True,
          on_select="rerun",
          key="chart_uscite_interattivo",
      )

      periodo_selezionato = None
      if (
          event_selezionato
          and "selection" in event_selezionato
          and event_selezionato["selection"]["points"]
      ):
      ...
      if periodo_selezionato:
        p_date_str = datetime.strptime(
            periodo_selezionato, "%Y-%m-%d"
        ).strftime("%d/%m/%Y")
        if tipo_aggregazione == "Settimanale":
          ...
          etichetta_periodo = (
              "Mese di"
              f" {datetime.strptime(periodo_selezionato, '%Y-%m-%d').strftime('%B %Y')}"
          )
        else:
      ...
      col_tot3.metric(
          "Ore in sella", f"{timedelta_to_str(tot_ore_periodo * 3600)}"
      )
      ...
      opzioni_tendina = {
          (
              f"{row['data_solo']} - {row['titolo_uscita']}"
              f" ({row[scelta_metrica]:.1f} {scelta_metrica})"
          ): row["id_str"]
          for _, row in df_filtrato_periodo.sort_values(
              "data_fmt", ascending=False
          ).iterrows()
      }
      ...
      if id_attivita_scelta:
        dati_uscita_corrente = df_g[
            df_g["id_str"] == id_attivita_scelta
        ].iloc[0]

        st.markdown(
            "#### 🚴 Dettaglio:"
            f" {dati_uscita_corrente['titolo_uscita']}"
        ...
        nuovo_nome_sec3 = st.text_input(
            ...
            key=f"input_nome_sec3_{id_attivita_scelta}",
        )
        if nuovo_nome_sec3 != dati_uscita_corrente["titolo_uscita"]:
          if st.button(
              "Salva nuovo nome", key=f"btn_salva_sec3_{id_attivita_scelta}"
          ):
            df_g.loc[
                df_g["id_str"] == id_attivita_scelta, "titolo_uscita"
            ] = nuovo_nome_sec3
            st.success(
                "Uscita rinominata da"
                f" '{dati_uscita_corrente['titolo_uscita']}' a"
                f" '{nuovo_nome_sec3}'!"
            )
            st.rerun()
        ...
        target_url_g = (
            f"https://intervals.icu/api/v1/activity/{clean_id_g}/streams"
        )
        ...
        with st.spinner("Caricamento traccia GPS in corso..."):
          ...
          if resp_str_g.status_code == 200:
            ...
            if lats_g and lons_g and len(lats_g) > 0:
              with st.expander(
                  "🗺️ Visualizza Mappa e Download GPX", expanded=False
              ):
                tipo_mappa = st.radio(
                    "Stile Mappa",
                    ["Satellite", "Standard"],
                ...
                fig_map.add_trace(
                    go.Scattermapbox(
                        lat=[lats_g[0], lats_g[-1]],
                        lon=[lons_g[0], lons_g[-1]],
                        mode="markers",
                ...
                if tipo_mappa == "Satellite":
                  mapbox_config = dict(
                      ...
                      center=dict(
                          lat=sum(lats_g) / len(lats_g),
                          lon=sum(lons_g) / len(lons_g),
                      ),
                      zoom=11,
                  )
                else:
                ...
                linee_gpx.extend(["    </trkseg>", "  </trk>", "</gpx>"])
                contenuto_gpx_uscita = "\n".join(linee_gpx)
                nome_file_gpx = "".join(
                    c
                    for c in dati_uscita_corrente["titolo_uscita"]
                    if c.isalnum() or c in (" ", "_", "-")
                ).strip().replace(" ", "_")
                ...
                b64_gpx = base64.b64encode(
                    contenuto_gpx_uscita.encode()
                ).decode()
                href_gpx = (
                    f'<a href="data:application/gpx+xml;base64,{b64_gpx}"'
                    f' download="{nome_file_gpx}.gpx" style="text-decoration:'
                    ' none;"><div style="background-color: #ff4b4b; color:'
                    " white; padding: 0.5rem 1rem; border-radius: 0.5rem;"
                    " text-align: center; font-weight: 600; margin-top:"
                    " 0.5rem; margin-bottom: 0.5rem;\">📥 Scarica Tracciato"
                    " GPX</div></a>"
                )
                st.markdown(href_gpx, unsafe_allow_html=True)
              else:
                st.warning(
                    "⚠️ Nessuna coordinata GPS valida disponibile per questa"
                    " specifica uscita su Intervals.icu."
                )
            else:
              st.error(
                  "Errore nel recupero flussi da Intervals (Status:"
                  f" {resp_str_g.status_code})"
              )
    else:
      st.info("Nessuna attività trovata nel range temporale scelto.")
