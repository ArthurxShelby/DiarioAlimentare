        ...
        if dati_sec4:
            ...
            if not df_s4.empty and 'start_date_local' in df_s4.columns:
                ...
                if not df_s4_filtrato.empty:
                    ...
                    # Estrazione corretta e pulita dai campi Intervals
                    ...
                    # Se l'API fornisce direttamente i campi riassuntivi o calcolati
                    val_if = float(m.get('intensity_factor') or 0.0)
                    if val_if == 0.0 and np_val > 0 and val_eftp > 0:
                    ...
                    # W' Bal: nel tuo debug ss_w_prime è in Joule o kJ? Verifichiamo la scala
                    w_bal_raw = float(m.get('ss_w_prime') or m.get('min_w_prime_balance') or m.get('w_prime_balance') or 0.0)
                    if w_bal_raw > 100:
                        val_wbal = w_bal_raw / 1000.0  # Se è in Joule (es. 2800 -> 2.8 kJ)
                    else:
                        val_wbal = w_bal_raw         # Se è già in kJ o decimale
