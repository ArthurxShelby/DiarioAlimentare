...
def salva_dati_disco():
    ...
    try:
        dati = {
            ...
            "utenti_ospiti": st.session_state.get("utenti_ospiti", {}),
        }
...
# Inizializzazione Gestione Ospiti Isolati (Credenziali e Mapping Atleta Dedicato)
if "utenti_ospiti" not in st.session_state:
    if dati_salvati and "utenti_ospiti" in dati_salvati:
        st.session_state.utenti_ospiti = dati_salvati["utenti_ospiti"]
    else:
        st.session_state.utenti_ospiti = {}

if "utente_loggato" not in st.session_state:
    st.session_state.utente_loggato = (
        None  # None = Admin / Libero, oppure Username Ospite
    )
...
# --- GESTIONE LOGIN / BLOCCO UTENTI OSPITI ISOLATI ---
if st.session_state.utente_loggato is not None:
    # Utente ospite loggato: blindato in modo isolato sul proprio profilo e dati dedicati
    username_ospite = st.session_state.utente_loggato
    atleta_assegnato = st.session_state.utenti_ospiti.get(username_ospite, {}).get(
        "atleta_collegato"
    )

    if not atleta_assegnato or atleta_assegnato not in st.session_state.atleti:
        st.error(
            "Errore critico: profilo atleta associato non trovato o rimosso dall'amministratore."
        )
    ...
    st.session_state.atleta_corrente = atleta_assegnato

    st.sidebar.warning(
        f"🔒 Accesso Utente Ospite: **{username_ospite}**\n\nProfilo Isolato: **{atleta_assegnato}**"
    )
    if st.sidebar.button("🚪 Esci (Logout)"):
        st.session_state.utente_loggato = None
        st.rerun()
else:
    # Schermata di autenticazione nella sidebar
    st.sidebar.header("Accesso Applicazione")
    modalita_accesso = st.sidebar.radio(
        "Seleziona modalità", ["Proprietario (Admin)", "Utente Ospite (Isolato)"]
    )

    if modalita_accesso == "Utente Ospite (Isolato)":
        if not st.session_state.utenti_ospiti:
            st.sidebar.info(
                "Nessun account ospite configurato dal proprietario. Accedi come Admin per crearne uno."
            )
            st.stop()

        user_input = st.sidebar.selectbox(
            "Seleziona Username", list(st.session_state.utenti_ospiti.keys())
        )
        pwd_input = st.sidebar.text_input("Password Personale", type="password")

        if st.sidebar.button("Accedi alla tua Area Personale"):
            dati_ospite = st.session_state.utenti_ospiti.get(user_input, {})
            pwd_salvata = dati_ospite.get("password", "")

            if pwd_salvata == "" and pwd_input == "":
                st.session_state.utente_loggato = user_input
                st.session_state.atleta_corrente = dati_ospite["atleta_collegato"]
                st.rerun()
            elif pwd_input == pwd_salvata and pwd_salvata != "":
                st.session_state.utente_loggato = user_input
                st.session_state.atleta_corrente = dati_ospite["atleta_collegato"]
                st.rerun()
            else:
                st.sidebar.error("Password errata o non valida.")
        st.stop()
    else:
...
# --- SEZIONE GESTIONE ATLETI E UTENTI OSPITI NELLA SIDEBAR (SOLO PER ADMIN) ---
if st.session_state.utente_loggato is None:
    st.sidebar.header("Gestione Atleti & Ospiti Isolati")
    ...
    with st.sidebar.expander("Gestione Profili e Accessi Ospite"):
        st.markdown("### 1. Crea Nuovo Atleta & Utente Ospite")
        nuovo_atleta_nome = st.text_input("Nome Profilo Atleta")
        username_ospite_nuovo = st.text_input("Username per Login Ospite")
        nuova_pwd_ospite = st.text_input(
            "Password per Utente Ospite", type="password"
        )

        if st.button("Crea Profilo & Credenziali Isolate"):
            nome_p = nuovo_atleta_nome.strip()
            user_p = username_ospite_nuovo.strip()

            if nome_p == "" or user_p == "":
                st.error("Inserisci sia il nome atleta che lo username ospite.")
            elif nome_p in st.session_state.atleti:
                st.warning("Esiste già un atleta con questo nome.")
            elif user_p in st.session_state.utenti_ospiti:
                st.warning("Esiste già un utente ospite con questo username.")
            else:
                # Creazione profilo atleta isolato
                st.session_state.atleti[nome_p] = {
                    "peso": 70.0,
                    "altezza": 175.0,
                    "eta": 30,
                    "genere": "Uomo",
                ...
                # Associazione credenziali ospite blindate al profilo atleta
                st.session_state.utenti_ospiti[user_p] = {
                    "password": nuova_pwd_ospite.strip(),
                    "atleta_collegato": nome_p,
                }
                st.session_state.atleta_corrente = nome_p
                salva_dati_disco()
                st.success(
                    f"Atleta '{nome_p}' e utente ospite '{user_p}' creati e associati con successo!"
                )
                st.rerun()

        st.markdown("---")
        st.markdown("### 2. Elenco Account Ospiti Attivi")
        if st.session_state.utenti_ospiti:
            for u, info_u in list(st.session_state.utenti_ospiti.items()):
                st.write(
                    f"- **User:** {u} ➔ **Atleta:** {info_u['atleta_collegato']}"
                )
            
            user_da_eliminare = st.selectbox(
                "Seleziona Utente Ospite da eliminare",
                list(st.session_state.utenti_ospiti.keys()),
            )
            if st.button("Elimina Account Ospite e Profilo", type="primary"):
                atleta_collegato_target = st.session_state.utenti_ospiti[
                    user_da_eliminare
                ]["atleta_collegato"]
                
                # Rimuove l'utente ospite
                del st.session_state.utenti_ospiti[user_da_eliminare]
                
                # Rimuove anche l'atleta associato se non è l'atleta principale
                if (
                    atleta_collegato_target in st.session_state.atleti
                    and atleta_collegato_target != "Atleta Principale"
                ):
                    del st.session_state.atleti[atleta_collegato_target]

                st.session_state.atleta_corrente = "Atleta Principale"
                salva_dati_disco()
                st.success(
                    f"Account ospite '{user_da_eliminare}' e relativo profilo eliminati."
                )
                st.rerun()
        else:
            st.info("Nessun account ospite configurato.")
...
# --- SEZIONE ALLENAMENTO INDIPENDENTE PER ATLETA ---
...
if st.session_state.utente_loggato is None:
                    ...
                    if df_nuovo is not None and not df_nuovo.empty:
                        ...
                        if st.button("Conferma e Aggiungi alla Banca Dati"):
                            ...
                            if len(presenti) < 4 and len(df_nuovo.columns) >= 4:
                                ...
                                df_nuovo = df_nuovo.rename(
                                    columns=col_mapping_pos
                                )
                                df_nuovo = df_nuovo.loc[
                                    :, ~df_nuovo.columns.duplicated()
                                ]
    ...
    # Per gli utenti ospiti, mostra la banca dati in sola lettura
    with st.expander("Visualizza Banca Dati Alimenti (Sola Lettura)", expanded=False):
