...
st.set_page_config(page_title="Diario Alimentare", page_icon="
\f1 \uc0\u55356 \u57167 
\f0 ", layout="centered")\
\
...
eta = st.sidebar.number_input("Et\'e0 (anni)", min_value=1, max_value=120, value=30)\
laf = st.sidebar.selectbox("Attivit\'e0 Fisica (LAF)", \
...
st.subheader("
\f1 \uc0\u10133 
\f0  Inserisci Alimento nel Diario")\
...
for p in pasti_lista:\
    ...
    with st.expander(f"
\f2 \uc0\u10132 
\f0  \{p\} (Totale Kcal: \{int(df_pasto['Kcal'].sum()) if not df_pasto.empty else 0\})", expanded=True):\
...
with st.expander("
\f1 \uc0\u55357 \u56770 \u65039 
\f0  Gestisci / Aggiungi Alimenti alla Banca Dati"):\
