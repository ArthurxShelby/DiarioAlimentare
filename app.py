  ...
  with col_target:
    with st.container(border=True):
      ...
      if not df_p.empty:
        ...
        st.caption(
            f"Totale: {p_kcal:.1f} kcal | C: {p_carb:.1f}g | P: {p_prot:.1f}g | G:"
            f" {p_gras:.1f}g"
        )
      ...
      for pasto in PASTI:
        ...
        if not df_p.empty:
          for _, row in df_p.iterrows():
            testo_riga = (
                f" - {row['Alimento']}: {row['gr/n']}g | Carbo:"
                f" {row['carbo']}g | Prot: {row['proteine']}g | Grassi:"
                f" {row['grassi']}g | {row['kcal']} kcal"
            )
  ...
  if st.button("Genera e Scarica PDF Intervallo (Traccia Totale)"):
    try:
      if data_inizio > data_fine:
        ...
        pdf_output.set_text_color(0, 0, 0)
        pdf_output.write(8, " kcal (Media giornaliera: ")
        if media_kcal > obj_kcal:
          pdf_output.set_text_color(220, 20, 60)
