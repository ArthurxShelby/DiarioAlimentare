...
with st.expander("📚 Gestione Avanzata Banca Dati Alimenti", expanded=False):
    ...
    with col_bd2:
        st.markdown("### 📥 Integrazione File (CSV, Excel/Numbers, PDF)")
        st.info("I file devono contenere colonne o dati per: Alimento, gr/n, carbo, proteine, grassi, kcal.")
        file_caricato = st.file_uploader("Carica file (CSV, XLSX, XLS, NUMBERS, PDF)", type=["csv", "xlsx", "xls", "numbers", "pdf"], key="uploader_banca_dati")
                ...
                if df_nuovo is not None:
                    ...
                    if st.button("Conferma e Aggiungi alla Banca Dati"):
                        ...
                        for c in df_nuovo.columns:
                            ...
                            elif "kcal" in c or "calorie" in c: mapping_colonne[c] = "kcal"
