    ...
    if len(st.session_state.atleti) > 1:
      ...
      if st.button("Conferma ed Elimina Atleta", type="primary"):
        if atleta_da_eliminare in st.session_state.atleti:
          ...
          if atleta_da_eliminare in st.session_state.password_atleti:
            del st.session_state.password_atleti[atleta_da_eliminare]
...
# --- SEZIONE ALLENAMENTO ISOLATA PER ATLETA ---
  ...
  with col_target:
    with st.container(border=True):
      ...
      if not df_p.empty:
        ...
        st.caption(
            f"Totale: {p_kcal:.1f} kcal | C: {p_carb:.1f}g | P: {p_prot:.1f}g |"
            f" G: {p_gras:.1f}g"
        )
  ...
  if st.button("Genera e Scarica PDF Giornaliero"):
    try:
      ...
      pdf_output.cell(
          0,
          10,
          f"Report Nutrizionale - {st.session_state.atleta_corrente}"
          f" ({data_str})",
          ln=True,
          align="C",
      )
      ...
      for pasto in PASTI:
        ...
        if not df_p.empty:
          for _, row in df_p.iterrows():
            testo_riga = (
                f" - {row['Alimento']}: {row['gr/n']}g | Carbo: {row['carbo']}g"
                f" | Prot: {row['proteine']}g | Grassi: {row['grassi']}g |"
                f" {row['kcal']} kcal"
            )
  ...
  if st.button("Genera e Scarica PDF Intervallo (Traccia Totale)"):
    try:
      if data_inizio > data_fine:
        ...
        tot_p_kcal, tot_p_carbo, tot_p_prot, tot_p_grassi = (
            0.0,
            0.0,
            0.0,
            0.0,
        ...
        for i in range(delta_giorni):
          ...
          if d_str in db_diario_atleta:
            d_kcal = sum([
                safe_float(db_diario_atleta[d_str][p]["kcal"].sum())
                for p in PASTI
                if not db_diario_atleta[d_str][p].empty
            ])
            d_carbo = sum([
                safe_float(db_diario_atleta[d_str][p]["carbo"].sum())
                for p in PASTI
                if not db_diario_atleta[d_str][p].empty
            ])
            d_prot = sum([
                safe_float(db_diario_atleta[d_str][p]["proteine"].sum())
                for p in PASTI
                if not db_diario_atleta[d_str][p].empty
            ])
            d_grassi = sum([
                safe_float(db_diario_atleta[d_str][p]["grassi"].sum())
                for p in PASTI
                if not db_diario_atleta[d_str][p].empty
            ])
        ...
        if dettaglio_periodo:
          for d_str, dk, dc, dp, dg in dettaglio_periodo:
            ...
            pdf_output.write(6, f" | Prot: {dp:.1f}g | Grassi: {dg:.1f}g\n")
        else:
          pdf_output.cell(
              0, 6, " - Nessun dato registrato nel periodo selezionato", ln=True
          )
